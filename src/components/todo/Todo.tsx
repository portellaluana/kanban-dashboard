...
function Todo() {
  ...
  function handleAddTask(event: FormEvent) {
    ...
    localStorage.setItem("tarefas-a-fazer", JSON.stringify([...title, newItem]));
  }

  function reoder<T>(list: T[], startIndex: number, endIndex: number) {
    const result = Array.from(list);
    const [reordenado] = result.splice(startIndex, 1);
    result.splice(endIndex, 0, reordenado);
    return result;
  }
  ...
  function ondragend(result: any) {
    ...
    localStorage.setItem("tarefas-a-fazer", JSON.stringify(items));
  }
